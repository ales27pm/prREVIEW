...
const createFallbackModule = (): Spec => ({
  async scanNetworks() {
    console.warn(
      '[WiFiSniffer] Returning empty network list because the native module is unavailable.'
    );
    return [];
  },
  async startCapture() {
  ...
  async sendDeauth() {
    console.warn(
      '[WiFiSniffer] Cannot send deauth packets because the native module is unavailable.'
    );
    return false;
  },
});
