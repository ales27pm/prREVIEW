...
export const PacketItem: React.FC<PacketItemProps> = ({ packet }) => {
  ...
  if (typeof packet.replayCounter === 'number') {
    metadata.push(`Replay #${packet.replayCounter}`);
  }
  if (typeof packet.reasonCode === 'number') {
    metadata.push(`Reason ${packet.reasonCode}`);
  }
  if (typeof packet.count === 'number' && packet.count > 1) {
    metadata.push(`${packet.count} frames`);
  }
  if (packet.ssid) {
    metadata.push(packet.ssid);
  }
  if (packet.security) {
    metadata.push(packet.security);
  }
  ...
  return (
    <View style={styles.packetItem}>
      ...
      <Text style={styles.packetMeta}>AP: {packet.bssid}</Text>
      <Text style={styles.packetMeta}>Client: {packet.clientMac}</Text>
      {metadata.length > 0 ? (
        <Text style={styles.packetMeta}>{metadata.join(' • ')}</Text>
      ) : null}
